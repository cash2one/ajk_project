...
class ProxyMiddleware(object):
    ...
    proxyfile = "C:/Users/edit-000127/Desktop/ajk/ajk/utils/validProxy.txt"
    ...
    def process_request(self, request, spider):
        ...
        #logging.info("proxy_change：{},redirect_urls:{}".format(request.meta.get('proxy_change'),request.meta.get('redirect_urls')))

        if request.meta.get('proxy_change'):
            ...
            if not  request.meta.get('redirect_urls'):
                request.meta['agent'] = random.choice(list(filter(lambda x: x.is_valid(),self.agent_list)))
                request.meta['proxy'] = request.meta['agent'].proxy
    ...
    def _new_request_from_response(self,request):
        ...
        new_request.meta['proxy_change'] = True
        return new_request
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status == 200:
            if "安居客".encode() in response.body:
            ...
            if response.headers.get('Location'):
                if response.headers.get('Location').decode()=='http://beijing.anjuke.com/404/':
                    agent.set_invalid()
            ...
            if "安居客".encode() in response.body: 
                ...
                logging.info("Proxy {} meet real unkonwn {} ".format(agent.proxy,reason))
    ...
    def process_exception(self, request, exception, spider):
        """Handle some connection error, make another request when these error happens
           The first version of the return response ', V1.1 in the change removed' return response '
        """        
        agent = request.meta.get('agent')
        if isinstance(exception,self.DONT_RETRY_ERRORS):
            logging.info("Failed proxy:{} for processing {}".format(request.meta['proxy'],request.url))
            agent.weaken()
            return self._new_request_from_response(request)
        # return response
...
class Agent(object):
    ...
    """
    def __init__(self,proxy, success=100, percentage=1, absolute_threshold=80, percentage_threshold=0.50):
